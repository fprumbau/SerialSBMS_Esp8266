...
void OTA::init(ESP8266WebServer& server, const char* host) {
  ...
  if(flashSize > 4000000) {
     ...
     _otaUpdater.setUpdaterUi("Title", "SBMS120 Solar Charger", "Build : 0.7.24", "Branch : master","Device info : ukn","mySerial.readString usage; Laengenpruefung sread; Reconnect on close;Integration ReconnectingWebSocket; Reparaturversuch, Debugschalter zu setzen, Validierung payload length");
